...
-spec to_json(session()) -> fen_json:json().
to_json(Session) ->
  #{ id => fen_json:encode_null(maps:get(id, Session))
   , token => maps:get(token, Session)
   , agent => fen_json:encode_null(maps:get(agent, Session))
   , created_at => fen_json:encode_date(maps:get(created_at, Session))
   , expires_at => fen_json:encode_date(maps:get(expires_at, Session))
   }.
...
from_json(Json) ->
  Now = fen_json:encode_date(calendar:universal_time()),
  ExpiresAt = fen_json:encode_date(expires_at()),
  try
    { ok
    , #{ id => fen_json:decode_null(maps:get(<<"id">>, Json, null))
       , token => maps:get(<<"token">>, Json, generate_token())
       , agent => fen_json:decode_null(maps:get(<<"agent">>, Json, null))
       , created_at =>
          fen_json:decode_date(maps:get(<<"created_at">>, Json, Now))
       , expires_at =>
          fen_json:decode_date(maps:get(<<"expires_at">>, Json, ExpiresAt))
       }
    }
  catch
    _:{badkey, Key} ->
